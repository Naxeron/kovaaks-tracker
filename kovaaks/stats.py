...
def _get_local_stats_incremental(stats_dir, current_files, cache_dict, now_dt):
    """Internal helper to parse new files and update cached stats."""
...
def _get_local_stats_full(stats_dir, current_files):
    """Internal helper to fully parse stats directory (fallback path)."""
    stats = {}
...
def _update_runs_today(stats, current_files, now_dt):
    """Calculate runs_today dynamically from filenames (very fast!)."""
    yesterday = now_dt - datetime.timedelta(days=1)
    today_str = now_dt.strftime("%Y.%m.%d")
    yesterday_str = yesterday.strftime("%Y.%m.%d")

    # Fast filtering of files matching today or yesterday
    matching_files = [f for f in current_files if (today_str in f or yesterday_str in f)]
...
def get_local_stats(stats_dir, cache_dict=None):
    ...
    if cache_dict is not None:
        stats = _get_local_stats_incremental(stats_dir, current_files, cache_dict, now_dt)
    else:
        stats = _get_local_stats_full(stats_dir, current_files)

    _update_runs_today(stats, current_files, now_dt)
    return stats
